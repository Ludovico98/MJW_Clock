...
name = "Firecracker Network"
password = "flammable07"
...
###############################################################################################################
delayMotor = 0.03499 #0.151 * 1.02 = 1/[(0.396rpm * 1000ppr) / 60] change this for speed of both motors
...
hrMinRatio = 11 # 12 hours including 0, change this to configure the ratio between disc
startHour = 16 #change start hour
endHour = startHour + 24 # 12 hours run time
###############################################################################################################
...
def main():
    ...
    try:
        while True:
            ...
            while startHour<hourStamp<endHour:#Running Mode
                running = True
                GPIO.output(enaMin, GPIO.LOW)
                GPIO.output(enaHr, GPIO.LOW)
            ...
            while not startHour<hourStamp<endHour:   #Sleep mode    
                running = False
                while GPIO.input(switchMin) == GPIO.HIGH:
                    minuteFast()
                minCount = -1
